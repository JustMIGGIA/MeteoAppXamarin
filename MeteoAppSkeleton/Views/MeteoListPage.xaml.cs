...
namespace MeteoAppSkeleton.Views
{
    public partial class MeteoListPage : ContentPage
    {
        ...
        {
            string locationName = await DisplayPromptAsync("Add location", "Insert a location name");

            Uri uri = new Uri("https://api.openweathermap.org/data/2.5/weather?q=" + locationName + "&units=metric&lang=it&appid=" + KEY);
            ...
            {
                DependencyService.Get<IMessage>().ShortAlert("Location doesn't exists!");
                return;
            }
            ...
            if (await App.Database.SaveItemAsync(location))
                ((MeteoListViewModel)BindingContext).add(location);
            else
                DependencyService.Get<IMessage>().ShortAlert("Location already present!");
        }
        ...
        private async void Button_Clicked(object sender, EventArgs e)
        {
            ...
            ((MeteoListViewModel)BindingContext).Locations.Remove(loc);
        }
    }
}